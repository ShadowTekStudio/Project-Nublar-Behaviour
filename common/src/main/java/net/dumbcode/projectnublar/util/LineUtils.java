...
public class LineUtils {
    ...
    public static double[] intersect(BlockPos position, BlockPos fromPos, BlockPos toPos, double yoff) {
        Vec3 from = new Vec3(fromPos.getX(), fromPos.getY(), fromPos.getZ()).add(0.5, yoff, 0.5);
        Vec3 to = new Vec3(toPos.getX(), toPos.getY(), toPos.getZ()).add(0.5, yoff, 0.5);
        Optional<Vec3> result = new AABB(position).clip(from, to);
        Optional<Vec3> reverseResult = new AABB(position).clip(to, from);
        if(result.isPresent() && reverseResult.isPresent()) {
            Vec3 normal = result.get();
            Vec3 reverse = reverseResult.get();
            if(position.equals(BlockPos.containing(from.x, from.y, from.z))) {
                normal = from;
            } else if(position.equals(BlockPos.containing(to.x, to.y, to.z))) {
                reverse = to;
            }
    ...
    public static List<BlockPos> getBlocksInbetween(BlockPos fromPos, BlockPos toPos, double offset) {
        ...
        Vec3 from = new Vec3(fromPos.getX() + 0.5, fromPos.getY() + offset, fromPos.getZ() + 0.5);
        Vec3 to = new Vec3(toPos.getX() + 0.5, toPos.getY() + offset, toPos.getZ() + 0.5);
