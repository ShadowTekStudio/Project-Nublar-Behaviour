...
public class AmberFeature extends Feature<FossilConfiguration> {
    public AmberFeature(Codec<FossilConfiguration> pCodec) {
        super(pCodec);
    }
     ...
     */
    public boolean place(FeaturePlaceContext<FossilConfiguration> pContext) {
        RandomSource randomsource = pContext.random();
        BlockPos blockpos = pContext.origin();
        WorldGenLevel worldgenlevel = pContext.level();
        FossilConfiguration fossilConfiguration = pContext.config();
        float f = randomsource.nextFloat() * (float) Math.PI;
        float f1 = (float) fossilConfiguration.size / 8.0F;
        int i = Mth.ceil(((float) fossilConfiguration.size / 16.0F * 2.0F + 1.0F) / 2.0F);
        ...
        for (int l1 = k; l1 <= k + j1; ++l1) {
            for (int i2 = i1; i2 <= i1 + j1; ++i2) {
                if (l <= worldgenlevel.getHeight(Heightmap.Types.OCEAN_FLOOR_WG, l1, i2)) {
                    return this.doPlace(worldgenlevel, randomsource, fossilConfiguration, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
                }
            }
        }
    ...
    protected boolean doPlace(WorldGenLevel pLevel, RandomSource pRandom, FossilConfiguration pConfig, double pMinX, double pMaxX, double pMinZ, double pMaxZ, double pMinY, double pMaxY, int pX, int pY, int pZ, int pWidth, int pHeight) {
        int i = 0;
        ...
        try (BulkSectionAccess bulksectionaccess = new BulkSectionAccess(pLevel)) {
            for (int j4 = 0; j4 < j; ++j4) {
                ...
                if (!(d9 < 0.0D)) {
                    ...
                    String period = FossilsConfig.getPeriod(l);
                    if(!FossilsConfig.testPeriodChance(period, pRandom)){
                        return false;
                    }
    ...
    public static boolean canPlaceOre(BlockState pState, Function<BlockPos, BlockState> pAdjacentStateAccessor, RandomSource pRandom, FossilConfiguration pConfig, BlockPos.MutableBlockPos pMutablePos) {
