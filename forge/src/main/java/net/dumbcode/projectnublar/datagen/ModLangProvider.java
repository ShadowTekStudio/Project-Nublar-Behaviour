...
//        ItemInit.ITEMS.getEntries().forEach(this::itemLang);
        EntityInit.ENTITIES.forEach(this::entityLang);
        BlockInit.BLOCKS.forEach(this::blockLang);
    ...
    protected void itemLang(DeferredSupplier<Item> entry) {
        if (!(entry.get() instanceof BlockItem) || entry.get() instanceof ItemNameBlockItem) {
            addItem(entry, checkReplace(entry));
        }
    }

    protected void blockLang(DeferredSupplier<Block> entry) {
        addBlock(entry, checkReplace(entry));
    }

    protected void entityLang(DeferredSupplier<EntityType<?>> entry) {
        addEntityType(entry, checkReplace(entry));
    }

    protected String checkReplace(DeferredSupplier<?> DeferredSupplier) {
        return Arrays.stream(DeferredSupplier.getId().getPath().split("_"))
                .map(this::checkReplace)
                .filter(s -> !s.isBlank())
                .collect(Collectors.joining(" "))
                .trim();
    }
    protected String checkReplace(FossilPiece DeferredSupplier) {
        return Arrays.stream(DeferredSupplier.name().split("_"))
                .map(this::checkReplace)
                .filter(s -> !s.isBlank())
                .collect(Collectors.joining(" "))
                .trim();
